...
if "apuntes" in chroma_client.list_collections():
    st.write("En el siguiente espacio puedes escribir una frase o pregunta y posteriormente consultar cuales son los 5 documentos más cercanos en la base de datos a esa frase")
    texto_consulta = st.text_input("Texto a consultar en la base de datos",placeholder="Escribe tu consulta")
    ...
    if boton_consulta:
        ...
        for index in range(len(documentos['ids'][0])):
            st.write(f"id: {documentos['ids'][0][index]}, distancia: {documentos['distances'][0][index]}\nTexto: {documentos['documents'][0][index]}")
